...
case "$ARCH" in
  x86_64|amd64)
    if [ "$OS_TYPE" = "linux" ]; then TARGET_TRIPLE="x86_64-unknown-linux-musl"; else TARGET_TRIPLE="x86_64-apple-darwin"; fi
    ;;
  aarch64|arm64)
    if [ "$OS_TYPE" = "linux" ]; then TARGET_TRIPLE="aarch64-unknown-linux-musl"; else TARGET_TRIPLE="aarch64-apple-darwin"; fi
    ;;
...
if [ -z "${BIN_PATH}" ]; then
  ...
  echo "Archive contents:"; tar -tzf "$TMP_DIR/owlsol.tar.gz" || true
  exit 1
fi
...
if ! command -v owlsol >/dev/null 2>&1 && ! echo "$PATH" | grep -q "$INSTALL_DIR"; then
  echo
  echo "⚠ $INSTALL_DIR not in PATH. Add to your shell rc:"
